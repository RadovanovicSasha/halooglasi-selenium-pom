...
- `support.ObstacleHandler` centralizes handling of known, removable UI obstacles (cookie banner, security notification modal) and explicit detection of external anti-bot challenges (Cloudflare/Turnstile). `BasePage.isVisible`/`clickWhenClickable` call it on a timeout: known obstacles are dismissed and the original wait/click is retried exactly once; a detected anti-bot challenge instead fails immediately with a distinct `AntiBotChallengeException`, so a Cloudflare block is never silently reported as an ordinary assertion/timeout failure. See [Project Limitations](#project-limitations-live-website).
- `BaseTest` provides the shared JUnit lifecycle (`@BeforeEach`/`@AfterEach`): driver creation, opening the site, dismissing known interstitials and failing fast on an anti-bot challenge (via `ObstacleHandler`) before each test, and quitting the driver after.
...
- **Cloudflare bot-verification can intercept any run.** The site is fronted by Cloudflare, which occasionally serves a "Verify you are human" / "Performing security verification" challenge page to automated ChromeDriver sessions instead of the real page. `ObstacleHandler` detects this explicitly (known challenge text and markup) and fails fast with `AntiBotChallengeException` instead of letting every test fail identically with an ambiguous "element not visible" message. The framework never attempts to click, solve, or otherwise bypass the challenge - detection exists purely so a Cloudflare block is reported clearly and isn't mistaken for a broken locator or a normal timeout. This remains an anti-bot/environment condition external to the framework, not a test or page-object defect, and isn't tied to a specific test case.
